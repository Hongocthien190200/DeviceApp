...
function Home() {
    ...
    const cards = [
        ...
        { title: 'Quản lý tài khoản', link: '/setting', color: '#FF1493', count: countAll?.user, icon: "fa-solid fa-users" },
    ];
    useEffect(() => {
        if (user?.accessToken) {
        ...
        <>
            <div className={cx('home')}>
                {cards.map((card, index) => (
                    <Link to={card.link} key={index} className={cx('card')} >
                        <div className={cx('circle')} style={{ backgroundColor: card.color }}>
